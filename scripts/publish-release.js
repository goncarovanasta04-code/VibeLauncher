...
async function main() {
  ...
  const releaseNotes = `### 🚀 VibeLauncher ${tagName} — Каталог сборок, независимые профили, авто-подбор Java и Potato PC

#### 📦 Полноценная поддержка и каталог сборок (Modrinth Modpacks)
- **Независимые инстансы:** каждая сборка устанавливается в собственную изолированную папку, что позволяет скачивать несколько копий или разные версии одного модпака.
- **Исправление каталога модов:** поиск модов внутри выбранной сборки теперь корректно использует базовую версию Minecraft и автоматически синхронизирует нужный загрузчик (Fabric, Forge, NeoForge).
- **Статус «Установлено»:** в каталоге сборок отображается статус установки и кнопка удаления.

#### ☕ Умный авто-подбор и загрузка Java
- Автоматическое определение необходимой версии Java (Java 8, 16, 17, 21) под конкретный Minecraft и установленные моды.
- Автоматическое скачивание и проверка совместимости пользовательской Java для исключения любых сбоев запуска.

#### 🗑️ Быстрое управление версиями
- Удаление скачанных сборок и версий в 1 клик прямо из выпадающего списка на главной странице и в менеджере версий.

#### ⚡ Оптимизация для слабых ПК (Potato PC Mode)
- Легковесный сборщик мусора G1GC с минимальным потреблением ОЗУ.
- Авто-настройка Minecraft options.txt (4 чанка, отключение теней/облаков) для максимального FPS.`
