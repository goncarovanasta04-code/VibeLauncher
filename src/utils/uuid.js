/**
 * Generates an offline Minecraft UUID from a username
 * Matches Java UUID.nameUUIDFromBytes(("OfflinePlayer:" + username).getBytes(StandardCharsets.UTF_8))
 */
export function generateOfflineUUID(username) {
  const hash = md5('OfflinePlayer:' + (username || 'Player'))
  // Format as Minecraft Java UUID v3
  // hash is 32 hex chars
  const p1 = hash.substring(0, 8)
  const p2 = hash.substring(8, 12)
  const p3 = '3' + hash.substring(13, 16) // version 3
